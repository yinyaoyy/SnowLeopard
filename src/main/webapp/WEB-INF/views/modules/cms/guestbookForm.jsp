...
<style type="text/css">
	...
	.qa-box .a-msg .detail-thumbs .closely-ban {
		...
		.qa-box .a-msg .detail-thumbs .closely-bannn {
		display: inline-block;
		background: transparent;
		margin-right: 15px;
		border: 1px solid #1296D6;
		...
		.qa-box .a-msg .detail-thumbs .closely-bannnn {
		display: inline-block;
		background: transparent;
		margin-right: 15px;
		border: 1px solid #1296D6;
		...
		/* //detail-main.detail-a.qa-box.qa-closely.closely-aaa
							a.qa-box.qa-closely.closely-aa */
		...
		$(".closelyTextareaBan").click(function(){//点击按钮提交
			...
			var arrss=$(this).attr("data-name");
		  	if(content.length == 0){
	            ...
	            $.ajax({
	                 ...
	                 data: ({'id':arrss,"conttent":content,'type':"guestbookForm"}),
	                 type:'post',
	                 success:function(data){
	                	if(data.data="success"){
	                		//alert("保存追答成功");
	                		$("#inputForm").attr("action","${ctx}/cms/guestbook/form");
	                		$("#inputForm").submit();
	                		$("#inputForm").attr("action","${ctx}/cms/guestbook");
	                	}else{
	                		//alert("存追答失败");
	                		$("#inputForm").attr("action","${ctx}/cms/guestbook/form");
	                		$("#inputForm").submit();
	            ...
	            $.ajax({
	                 ...
	                 success:function(data){
	                	if(data.data="success"){
	                		...
	                		//alert("保存追答成功");
	                		window.location.href="${ctx}/cms/guestbook/form?id="+data.id;
	                	}else{
	                		window.location.href="${ctx}/cms/guestbook/form?id="+data.id;
	                	}
		 ...
		 $(".closely-bannnn").click(function(){
			 ...
			 if(window.confirm('要删除改记录吗')){
				 window.location.href="${ctx}/cms/guestbookcommentre/delete?id="+arrss; 
			 }else{
		 ...
		 $(".closely-aaaa").click(function(){
			 var arrss=$(this).attr("data-name");
			 if(window.confirm('要删除改记录吗')){
				 window.location.href="${ctx}/cms/guestbookcommentre/deleteguestbookcommentre?id="+arrss; 
			 }else{
				 $(".closely-aaaa").focus();
			 }
		 ...
		 $("#closely-btn-delete").click(function() {
			    var statu = confirmx('确认要删除该留言吗？', jsCtx + "/cms/guestbook/delete?id=" + $("#complaintid").val()+ "");
		  });
		  //修改追答内容
...
<ul class="nav nav-tabs">
			<li>
				<a href="${ctx}/cms/guestbook/">留言列表</a>
			</li>
			<li class="active">
				<a href="${ctx}/cms/guestbook/form?id=${guestbook.id}">留言<!--
					--><shiro:hasPermission name="cms:guestbook:edit">${guestbook.delFlag eq '2'?'审核':'查看'}</shiro:hasPermission><!--
					--><shiro:lacksPermission name="cms:guestbook:edit">查看</shiro:lacksPermission>
				</a>
			</li>
		</ul><br/>
<div class="wrapper">
			...
			<div class="content-wrapper">
				<div class="detail-content">
					<div class="detail-main">
						<div class="detail-q">
							<div class="q-title">${guestbook.title}</div>
							<div class="q-msg">${guestbook.content}</div>
							<div class="qa-box">
								<div class="detail-thumbs">
									<div class="q-time">
										留言人：<span id="q-time">${guestbook.name}</span>&nbsp;&nbsp;&nbsp;咨询时间：<span id="q-time"><fmt:formatDate value="${guestbook.createDate}" pattern="yyyy-MM-dd HH:mm:ss"/></span>
									</div>
									<br/>
									<div class="q-time">
										<input class="closely-btn" id="closely-btn" type="button" value="回复"/>
									...
									<div class="reClosely">
										<textarea name="reContent" class="closelyTextarea" cols="2"></textarea>
						...
						<div class="detail-a">
							...
							<c:forEach var="comment" items="${guestbook.commentList }">
							<div class="qa-box">
								<div class="a-msg">
									<div class="name">
										<span class="userInfo">${comment.createUser.name }</span> <span class="userInfo"><!-- 法律服务人员 --></span>
										<span style="float: right;">
											<c:if test="${comment.guestbookEvaluate.prescription eq '0'}">
												<img src="${ctxStatic}/images/star_0.png" alt="">
											</c:if>
											<c:if test="${comment.guestbookEvaluate.prescription eq '1'}">
												<img src="${ctxStatic}/images/star_1.png" alt="">
											</c:if>
											<c:if test="${comment.guestbookEvaluate.prescription eq '2'}">
												<img src="${ctxStatic}/images/star_2.png" alt="">
											</c:if>
											<c:if test="${comment.guestbookEvaluate.prescription eq '3'}">
												<img src="${ctxStatic}/images/star_3.png" alt="">
											</c:if>
											<c:if test="${comment.guestbookEvaluate.prescription eq '4'}">
												<img src="${ctxStatic}/images/star_4.png" alt="">
											</c:if>
											<c:if test="${comment.guestbookEvaluate.prescription eq '5'}">
												<img src="${ctxStatic}/images/star_5.png" alt="">
											</c:if>
										</span>
									</div>
...
<%-- 									         <input class="closely-aa" type="button" value="编辑" data-name="${commentre.id}" style="display: inline-block;background: transparent;margin-right: 15px;border: 1px solid #1296D6;color: #1296D6;border-radius: 4px;text-align: center;width: 50px;height: 22px;line-height: 0px;cursor: pointer;right: -93px; margin-left: 1358px;"/>
 --%>									       </c:otherwise> 
						        ...
						        <div class="detail-thumbs">
						             <div class="re-time">回复时间：
						                <span id="re-time"><fmt:formatDate value="${comment.createDate}" pattern="yyyy-MM-dd HH:mm:ss"/></span>
						             </div>
						             <div id="good" class="thumbsUp" data-thumbs="true" data-num="0">
						                <img src="${ctxStatic}/images/good.png" alt=""> 
						                  <span id="goodConut">( ${comment.thumbsUpTrue})</span>
						             </div>
						             <div id="diss" class="thumbsUp" data-thumbs="false" data-num="0">
						                <img src="${ctxStatic}/images/diss.png" alt=""> 
						                   <span id="dissConut">( ${comment.thumbsUpFalse} )</span>
						             </div>
						        </div>
							</div>
							</c:forEach>
