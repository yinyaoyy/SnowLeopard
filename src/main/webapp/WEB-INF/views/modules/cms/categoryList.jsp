			...
			<tbody>
				<c:forEach items="${list}" var="tpl">
					<tr id="${tpl.id}" pId="${tpl.parent.id ne '1'?tpl.parent.id:'0'}">
						...
						<td>
							<%-- <a href="${pageContext.request.contextPath}${fns:getFrontPath()}/list-${tpl.id}${fns:getUrlSuffix()}" target="_blank" title="访问"><i class="fa fa-fw fa-eye"></i></a> --%>
							<shiro:hasPermission name="cms:category:edit">
								<a href="${ctx}/cms/category/form?id=${tpl.id}" title="修改">修改</a>
								<a href="${ctx}/cms/category/delete?id=${tpl.id}"
									onclick="return confirmx('要删除该栏目及所有子栏目项吗？', this.href)"
									title="删除">删除</a>
								<a href="${ctx}/cms/category/form?parent.id=${tpl.id}"
									title="添加下级栏目">添加栏目</a>
							</shiro:hasPermission>
						</td>
					</tr>
