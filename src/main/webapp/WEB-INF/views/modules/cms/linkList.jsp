	...
	<form:form id="searchForm" modelAttribute="link" action="${ctx}/cms/link/" method="post" class="form-inline">
		...
		<div class="form-group">
				...
				<sys:treeselect id="category" name="category.id" value="${link.category.id}" labelName="category.name" labelValue="${link.category.name}"
					title="栏目" url="/cms/category/treeData" module="link" notAllowSelectRoot="false" cssClass="input-small form-control w200"/>
		</div>
		<div class="form-group">
				<label class="control-label">名称：</label>
				<form:input path="title" htmlEscape="false" maxlength="50" class="input-small form-control w200"/>
		</div>
		<div class="form-group">
		...
		<c:forEach items="${page.list}" var="link">
			<tr>
				...
				<shiro:hasPermission name="cms:link:edit"><td>
    				<a href="${ctx}/cms/link/form?id=${link.id}" title="修改"><i class="fa fa-fw fa-pencil"></i></a>
					<a href="${ctx}/cms/link/delete?id=${link.id}${link.delFlag ne 0?'&isRe=true':''}&categoryId=${link.category.id}" onclick="return confirmx('确认要${link.delFlag ne 0?'发布':'删除'}该链接吗？', this.href)" title="${link.delFlag ne 0?'发布':'删除'}">${link.delFlag ne 0?'<i class="fa-paper-plane"></i>':'<i class="fa fa-fw fa-trash"></i>'}</a>
