...
<head>
	...
	<script type="text/javascript">
		$(document).ready(function() {
			var activeIframe = $(top.document.body).find('.tab-pane.active').children('iframe').attr('id');
			$("input[name=id]").each(function(){
				var articleSelect = null;
				if (top.iframe_41){
					articleSelect = top.iframe_41.cmsMainFrame.articleSelect;
				}else{
					articleSelect = top.iframe_41.articleSelect;
				}
				...
				$(this).click(function(){
					...
					if (top.iframe_41.cmsMainFrame){
						top.iframe_41.cmsMainFrame.articleSelectAddOrDel(id, title);
					}else{
						top.iframe_41.articleSelectAddOrDel(id, title);
					}
				});
			});
		});
	...
	<form:form id="searchForm" modelAttribute="article" action="${ctx}/cms/article/selectList" method="post" class="breadcrumb form-search form-inline">
		...
		<div class="form-group">
				...
				<sys:treeselect id="category" name="category.id" value="${article.category.id}" labelName="category.name" labelValue="${article.category.name}"
					title="栏目" url="/cms/category/treeData" module="article" cssClass="input-small form-control w200"/>
		</div>
		<div class="form-group">
