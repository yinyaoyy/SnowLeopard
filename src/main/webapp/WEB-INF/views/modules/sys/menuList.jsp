			...
			<tbody>
				<c:forEach items="${list}" var="menu">
					<tr id="${menu.id}"
						...
						<td nowrap style="text-align:left;"><i
						...
						<td>${menu.languageKey}</td>
						<shiro:hasPermission name="sys:menu:edit">
							<td nowrap><a href="${ctx}/sys/menu/form?id=${menu.id}"
								title="修改">修改</a> <a
								href="${ctx}/sys/menu/delete?id=${menu.id}&langKey=${menu.languageKey}"
								onclick="return confirmx('要删除该菜单及所有子菜单项吗？', this.href)"
								title="删除">删除</a> <a
								href="${ctx}/sys/menu/form?parent.id=${menu.id}"
								style="width: 80px;" title="添加下级菜单">添加下级菜单</a></td>
						</shiro:hasPermission>
					</tr>
				</c:forEach>
			</tbody>
