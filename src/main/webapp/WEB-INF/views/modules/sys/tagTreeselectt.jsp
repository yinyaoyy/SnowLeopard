...
<head>
	...
	<script type="text/javascript">
		var key, lastValue = "", nodeList = [], isUser = getQueryString("isUser", "${url}");
		var tree, setting = {view:{selectedMulti:false,dblClickExpand:false},check:{enable:"${checked}",nocheckInherit:true},
				async:{enable:(isUser==12),url:"${ctx}/sys/user/treeData",autoParam:["id=officeId"]},
		...
		$(document).ready(function(){
			$.get("${ctx}${url}${fn:indexOf(url,'?')==-1?'?':'&'}&extId=${extId}&isAll=${isAll}&module=${module}&t="
				...
				// 默认不展开一级节点(如果展开，将循环中的第一个boolean参数改为true)
...
<body>
	...
	<div class="treeShowHideButton" onclick="search('1');" style="position:absolute;right:8px;top:5px;cursor:pointer;">
