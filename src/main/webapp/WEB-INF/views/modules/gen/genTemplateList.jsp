...
<body>
	...
	<form:form id="searchForm" modelAttribute="genTemplate" action="${ctx}/gen/genTemplate/" method="post" class="breadcrumb form-search">
		...
		<label class="control-label">分类 ：</label><form:select path="category" class="input-medium">
			<form:option value=""></form:option>
			<form:options items="${fns:getDictList('gen_category')}" itemLabel="label" itemValue="value" htmlEscape="false"/>
		</form:select>
		<label class="control-label">名称 ：</label><form:input path="name" htmlEscape="false" maxlength="50" class="input-medium"/>
	...
	<table id="contentTable" class="table table-striped table-bordered table-condensed">
		...
		<c:forEach items="${page.list}" var="genTemplate">
			<tr>
				...
				<td>${fns:getDictLabels('', 'gen_category', genTemplate.category,'')}</td>
				<td>${fns:abbr(genTemplate.remarks, 100)}</td>
				<shiro:hasPermission name="gen:genTemplate:edit"><td>
    				<a href="${ctx}/gen/genTemplate/form?id=${genTemplate.id}" title="修改"><i class="fa fa-fw fa-pencil"></i></a>
					<a href="${ctx}/gen/genTemplate/delete?id=${genTemplate.id}" onclick="return confirmx('确认要删除该代码模板吗？', this.href)" title="删除"><i class="fa fa-fw fa-trash"></i></a>
