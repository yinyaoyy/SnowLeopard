...
<body>
	<ul class="nav nav-tabs">
		<li><a href="${ctx}/info/judiciaryUser/">司法所工作人员列表</a></li>
		<li class="active"><a href="${ctx}/info/judiciaryUser/form?id=${judiciaryUser.id}">司法所工作人员<shiro:hasPermission name="info:judiciaryUser:edit">${not empty judiciaryUser.id?'修改':'添加'}</shiro:hasPermission><shiro:lacksPermission name="info:judiciaryUser:edit">查看</shiro:lacksPermission></a></li>
	</ul><br/>
	<form:form id="inputForm" modelAttribute="judiciaryUser" action="${ctx}/info/judiciaryUser/save" method="post" class="form-horizontal">
		<form:hidden path="id"/>
		...
		<div class="row">
			<div class="col-xs-3">
			    ...
			    <form:select path="roleId" class="input-xlarge form-control ">
			...
			<label class="control-label">开始执业时间：</label>
				<input name="practisingTime" type="text" readonly="readonly" maxlength="20" class="form-control "
					value="<fmt:formatDate value="${judiciaryUser.practisingTime}" pattern="yyyy-MM-dd"/>"
			...
			<div class="col-xs-3">
				<div class="form-group">
			<label class="control-label">是否兼职人民调解员：</label>
				<form:select path="isPeopleMediation" class="input-xlarge form-control ">
				   <option value="" selected="selected">请选择</option>
					<form:options items="${fns:getDictList('yes_no')}" itemLabel="label" maxlength="1" itemValue="value" htmlEscape="false"/>
				</form:select>
				</div>
		</div>
		</div>
		<div class="row">
			<div class="col-xs-3">
				<div class="form-group">
			<label class="control-label">归属司法所：</label>
			...
			<div class="col-xs-3">
				<div class="form-group">
			      <label class="control-label">头像路径：</label>
					<input type="hidden" id="imageUrl" name="imageUrl" value="${judiciaryUser.imageUrl}" />
			         <sys:ckfinder input="imageUrl" type="thumb" uploadPath="/info/judiciaryUser" selectMultiple="false" />
				</div>
			</div>
		</div>
		<div class="row">
		...
		<div class="form-actions" style="margin-left:10px;">
			<shiro:hasPermission name="info:judiciaryUser:edit"><input id="btnSubmit" class="btn btn-primary" type="submit" value="保 存"/>&nbsp;</shiro:hasPermission>
