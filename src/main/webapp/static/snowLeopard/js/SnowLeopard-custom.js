...
(function($, window, undefined) {
	...
	$(document).ready(function() {
		...
		if(public_vars.$mainFooter.hasClass('sticky')) {
			stickFooterToBottom();
			$(window).on('SnowLeopard.resized', stickFooterToBottom);
		}
...
//		}
		...
		$('body').on('click', 'a[rel="go-top"]', function(ev) {
			ev.preventDefault();
			var obj = {
				pos: $('.tab-pane iframe').contents().scrollTop()
			};
			...
			TweenLite.to(obj, .3, {
				...
				ease: Power4.easeOut,
				onUpdate: function() {
					$('.tab-pane iframe').contents().scrollTop(obj.pos);
				}
			});
		});
		...
		if(public_vars.$userInfoMenu.length) {
			public_vars.$userInfoMenu.find('.user-info-menu > li').css({
				maxHeight: public_vars.$userInfoMenu.find('li').outerHeight() - 1
			});
		}
		...
		// Register Form Label Focusing 2016-9-27
		$(".modal-body .form-group:has(label)").each(function(i, el) {
			var $this = $(el),
				$label = $this.find('label'),
				$input = $this.find('.form-control');
...
function ps_init() {
	$('#main-menu').find('.expanded').removeClass('expanded').find('ul').hide()
	if(isxs())
		return;
