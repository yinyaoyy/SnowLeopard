...
(function($, window, undefined) {
	...
	$(document).ready(function() {
			...
			$(el).on('click', function(ev) {
				...
				if($.isFunction($.fn.perfectScrollbar)) {
					setTimeout(function() {
						...
						$(window).trigger('SnowLeopard.resize');
					}, 1);
				}
			});
		});
		//点击弹出菜单选项关闭菜单
		$('a[data-toggle="dropdown"]').each(function(i, el)
		{
			$(el).siblings('ul').find('li a').on('click', function(ev)
			{
		...
		$('body').on('click',function(e){
			//e.preventDefault();
			var isopen = $('.dropdown').hasClass('open')||$('.dropdown', window.parent.document).hasClass('open')
			if(isopen){
