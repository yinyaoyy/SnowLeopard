...
(function ($) {
    ...
    $.flavr = function () {
        ...
        var L = function () {
            ...
            var rezw = $('<div class="flavr-resizeWindow"></div>')
            ...
            if (w.settings.canMove) {
                var M = false;
                var Rx, Ry;
                i.unbind('mousedown').bind('mousedown', function (e) {
                    console.log('inmove')
                    e.stopPropagation();
                    i.css({
                        cursor: 'move'
                    })
                    Rx = e.pageX - (parseInt($(".flavr-fixer").css("left")) || 0);
                    Ry = e.pageY - (parseInt($(".flavr-fixer").css("top")) || 0);
                    M = true;
            ...
            if (w.settings.resizewindow) {
                i.append(rezw)
                var R = false;
                var Rx, Ry;
                rezw.unbind('mousedown').bind('mousedown', function (e) {
                    e.stopPropagation();
                    R = true;
                    $(document).unbind('mousemove').bind('mousemove', function (e) {
                        e.stopPropagation();
                        if (R) {
                            Rx = parseInt(i.offset().left) - 5; //outer距顶宽度 -5以使鼠标包含在内
                            Ry = parseInt(i.offset().top) - 5; //outer距顶高度
                            var boxH = e.clientY - Ry;
                            var boxW = e.clientX - Rx;
                            j.css({
                                height: boxH,
                                width: boxW
                            });
            ...
            if (u !== false) w.show()
            if (A(w.settings.windowSize)) {
                j.css({
                    width: w.settings.windowSize.width,
                    height: w.settings.windowSize.height,
                })
                o.css({
                    height: j.outerHeight() - s.outerHeight() - n.outerHeight() - 30
                })
            }
            i.css({
                position: 'absolute',
                margin: 0,
                left: ($(window).width() - j.width()) / 2,
                top: ($(window).height() - j.height()) / 2
            })
        };
        w.show = function () {
