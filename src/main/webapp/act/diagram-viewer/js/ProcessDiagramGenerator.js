...
var ProcessDiagramGenerator = {	
	...
	drawDiagram: function(processDefinitionId,name) {
		// Hide all diagrams
		...
		if (processDefinitionId.indexOf(":") < 0) {
			...
			this._drawDiagram.apply({processDefinitionId: processDefinitionId,name:name});
		}
	},
	_drawDiagram: function() {
		var processDefinitionId = this.processDefinitionId;
		
	var namees = this.name;
		
		ProcessDiagramGenerator.addBreadCrumbsItem(processDefinitionId,namees);
	...
	addBreadCrumbsItem: function(processDefinitionId , name){
	...
	showFlowInfo: function(flow){
		...
		var TPL_FLOW_INFO = '' 
				+ '<div><b>流程来源Id</b>: {sourceActivityId}</div>'
				+ '<div><b>指定流程Id</b>: {destinationActivityId}</div>'
				+ '<div><b>是否默认</b>: {isDefault}</div>'
				+ '<div><b>是否有条件的</b>: {isConditional}</div>'
				+ '<div><b>是否突出</b>: {isHighLighted}</div>';
