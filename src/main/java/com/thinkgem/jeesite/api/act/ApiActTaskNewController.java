...
@RequestMapping(value = "/api/100/530")
public class ApiActTaskNewController {
	...
	private static final Logger log = LoggerFactory.getLogger(ApiActTaskNewController.class);
	/**
	 * 获取待办列表
    ...
    public ServerResponse getLegalOfficePerson(BaseForm<Map> form){
    	...
    	try {
    		...
    		if(result==null) {
    			...
    			//legalList = getOfficeUser("1".equals(type)?"2":"3", id ,areaId);
     ...
     */
	@RequestMapping("/100")
    public ServerResponse getUserbyOffice(BaseForm<Map> form){
    	ServerResponse result = null;
    	...
    	try {
    		form.initQueryObj(Map.class);
    		id = String.valueOf(form.getQueryObj().get("id"));
//    		Office office=	systemService.getSysUserOfficeInfo(id,"1");
    		list= systemService.findUserByOfficeId(id);
	...
	@RequestMapping("/120")
    public ServerResponse getInfoByQRCode(BaseForm<Map> form){
		JSONObject bject=(JSONObject) JSONObject.parse(form.getQuery());
		String procInsId=bject.getString("procInsId");
		Map<String,Object> map=new HashMap();
		
		if(StringUtils.isNoneBlank(procInsId)){
			map = actTaskService.getInfoByQRCode(bject.getString("procInsId"));	
		}
