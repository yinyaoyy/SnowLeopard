 ...
 */
@Service
public class NmgsftIdtPersonsService extends NmgsftsjjhptService {
	 ...
	 * 列名key.columns
	 */
	private static final String PROPERTIES_KEY = "idt_persons";
	...
	public List<Map<String, Object>> getParam() {
		...
		List<Map<String, String>> idtOrgsList = nmgsftsjjhptDao.selectIdtOrgs(DateUtils.getDate());
		//获取默认分页设置
		//从配置文件中读取指定key
		...
		for (int i = 0; i < idtOrgsList.size(); i++) {
			param = Maps.newHashMap();
			for (String key : idtOrgsList.get(i).keySet()) {
				//设置参数
				param.put(key, idtOrgsList.get(i).get(key));
			}
			//补充通用参数
