		...
		if (model != null) {
			try {
				...
				ObjectNode editorJsonNode = (ObjectNode) objectMapper.readTree(new String(repositoryService.getModelEditorSource(model.getId()),
						"utf-8"));
				JsonNode json = editorJsonNode.findValue("properties");
				 ((ObjectNode)json).put("process_id", model.getKey());
				 User currentUser = UserUtils.getUser();
				 ((ObjectNode)json).put("process_author",currentUser.getName());
				 String metaInfo = model.getMetaInfo();
				 JSONObject jsonObject = JSONObject.parseObject(metaInfo);
				 String jsid = jsonObject.getString("description");
				 ((ObjectNode)json).put("documentation", jsid);
				 ((ObjectNode)json).put("name", model.getName());
