...
//			logger.debug("File to object: {}", pathName);
	...
	public static GenConfig getConfig(){
		...
		if(CacheUtils.get("genConfig")!=null){
			config=(GenConfig) CacheUtils.get("genConfig");
		}else{
			config=fileToObject("config.xml", GenConfig.class);
			CacheUtils.put("genConfig",config);
		}
		return config;
	}
