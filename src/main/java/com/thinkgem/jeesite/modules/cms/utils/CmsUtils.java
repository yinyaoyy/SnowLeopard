...
public class CmsUtils {
	...
	public static List<Site> getSiteList(){
		...
		//if (siteList == null){
			...
			List<Site> siteList = page.getList();
    ...
    public static void setDefaultSite(HttpServletResponse response,String userId){
    	List<Site> list=CmsUtils.getSiteList();
		if(list.size()>0){
			UserUtils.putCache("siteId"+userId, list.get(0).getId());
			// 保存到Cookie中，下次登录后自动切换到该站点
			CookieUtils.setCookie(response, "siteId"+userId, list.get(0).getId());
		}
    }
}